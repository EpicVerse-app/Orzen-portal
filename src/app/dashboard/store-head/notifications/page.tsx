...
import { Bell, CheckCircle, XCircle, Package, Truck, Clock, ShieldCheck, ChevronRight } from 'lucide-react'
...
export default function StoreHeadNotificationsPage() {
  ...
  function NotifCard({ n }: { n: Notification }) {
    ...
    const inner = (
      <div className={`bg-white rounded-2xl border shadow-sm px-4 py-4 flex items-start gap-3 transition-all ${
        !n.is_read ? 'border-orange-100 bg-orange-50/20' : 'border-gray-100'
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex items-center gap-1 shrink-0 mt-0.5">
              {!n.is_read && <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: '#570439' }} />}
    ...
    if (n.order_id) {
      return <Link href={`/dashboard/store-head/orders/${n.order_id}`}>{inner}</Link>
    }
    return inner
  }
  ...
  return (
    <div className="px-4 sm:px-6 py-5 max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: '#57043918' }}>
          <ShieldCheck className="w-5 h-5" style={{ color: '#570439' }} />
        </div>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Notifications</h1>
          <p className="text-sm text-gray-400 mt-0.5">Order requests and branch updates</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-20 text-center">
          ...
          <p className="text-xs text-gray-400 mt-1">Order requests and approvals will appear here</p>
        </div>
      ) : (
