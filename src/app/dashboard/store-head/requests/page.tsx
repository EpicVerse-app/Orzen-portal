...
export default function StoreHeadRequestsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!prof || prof.role !== 'store_head') return
      setProfile(prof)
    ...
    <div className="px-4 sm:px-6 py-5 max-w-3xl mx-auto">
      ...
      {orders.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-20 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">No orders waiting for approval</p>
        </div>
      ) : (
