...
export default function SuperNotificationsPage() {
  ...
  function NotifCard({ n }: { n: Notification }) {
    ...
    if (n.order_id) {
      return <Link href={`/dashboard/super/orders/${n.order_id}`}>{inner}</Link>
    }
    return inner
  }
    ...
    <div className="px-4 sm:px-6 py-5 max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">New orders, approvals and fulfilment updates</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-20 text-center">
          ...
          <p className="text-xs text-gray-400 mt-1">New orders and updates will appear here</p>
        </div>
      ) : (
