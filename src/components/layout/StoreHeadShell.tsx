...
export default function StoreHeadShell({ user, children, logoUrl }: Props) {
  ...
  const initials = user.full_name?.split(' ').map((n: string) => n[0]).join('').slice(0, 2).toUpperCase() || 'SH'
  ...
  function NavItem({ label, path, icon: Icon, exact }: typeof NAV[0]) {
    ...
    return (
      <div className="relative nav-item mx-2 rounded-xl">
        {active && (
          <span className="absolute left-0 top-1.5 bottom-1.5 w-[3px] rounded-r-full animate-scale-in" style={{ backgroundColor: GOLD }} />
        )}
        <Link
          href={path}
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full" style={{ backgroundColor: MAROON }}>
      ...
      <div className="px-5 pt-5 pb-4 border-b border-white/10">
        ...
        <p className="text-sm font-bold text-white truncate">{branch?.name || 'No branch assigned'}</p>
      ...
      <div className="px-3 pb-2">
        <div className="rounded-xl px-4 py-3 hover:bg-black/30 transition-colors cursor-pointer" style={{ backgroundColor: 'rgba(0,0,0,0.25)' }}>
      ...
      <div className="border-t border-white/10 px-3 py-3">
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl hover:bg-white/5 transition-colors" style={{ backgroundColor: 'rgba(255,255,255,0.07)' }}>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      {/* ── Top Bar ── */}
      <header className="h-14 flex items-center px-4 sm:px-6 gap-3 fixed top-0 left-0 right-0 z-50 shadow-md" style={{ backgroundColor: MAROON }}>
        ...
        <div ref={notifRef} className="relative shrink-0">
          <button onClick={() => { setShowNotifs(!showNotifs); setShowProfile(false) }}
            ...
            <Bell className={`w-5 h-5 ${bellRing ? 'animate-bell' : ''}`} style={{ color: 'rgba(255,255,255,0.7)' }} />
            {unreadCount > 0 && (
          ...
          {showNotifs && (
            <div className="animate-dropdown absolute right-0 top-11 w-72 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden">
              ...
              <div className="border-t border-gray-100 px-4 py-2">
                <Link href="/dashboard/store-head/notifications" onClick={() => setShowNotifs(false)}
          ...
          {showProfile && (
            <div className="animate-dropdown absolute right-0 top-11 w-56 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-50">
              <div className="px-4 py-3 border-b border-gray-100 bg-gray-50">
                ...
                <p className="text-xs text-gray-500">Store Head</p>
                <p className="text-xs text-gray-400 mt-0.5">{branch?.name}</p>
              </div>
              <div className="py-1">
                <Link href="/dashboard/store-head" onClick={() => setShowProfile(false)}
