...
export default function SuperOrderActions({ orderId, companyId, approverId, approverRole, branchId }: Props) {
  ...
  async function handle(action: 'approved' | 'rejected') {
    ...
    const { error } = await supabase
      .from('orders')
      .update({ status: action, approved_by: approverId, approved_by_role: approverRole })
      .eq('id', orderId)
    ...
    router.push('/dashboard/super/requests')
    router.refresh()
  }
